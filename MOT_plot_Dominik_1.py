...
def data_processing(data_files):
...
def plot_MOT_Load(data):
    ...
    plt.plot(np.arange(np.shape(data)[2]),summed_data)
...
def plot_Temp(data,filename,camera_pixel_factor_x=6.7*um,camera_pixel_factor_y=6.7*um):
    ...
    TOF_step=0.25*1e-3
    p_x=np.polyfit(np.square(np.arange(data.shape[2])*TOF_step),np.square(width[0,:]),1)
    p_y=np.polyfit(np.square(np.arange(data.shape[2])*TOF_step),np.square(width[1,:]),1)
    print(p_x,p_y)
    
    #Calculate the Temperature in mK:
    Temp_x=calc_Temp(p_x[0])*1e3
    Temp_y=calc_Temp(p_y[0])*1e3
    print(Temp_x,Temp_y)
    
    plt.plot(np.square(np.arange(data.shape[2])*TOF_step),np.square(width[0,:]))
    plt.plot(np.square(np.arange(data.shape[2])*TOF_step),np.square(np.arange(data.shape[2])*TOF_step)*p_x[0]+p_x[1])
    plt.plot(np.square(np.arange(data.shape[2])*TOF_step),np.square(width[1,:]))
    plt.plot(np.square(np.arange(data.shape[2])*TOF_step),np.square(np.arange(data.shape[2])*TOF_step)*p_y[0]+p_y[1])
...
def plot_Temp2(data,filename,camera_pixel_factor_x=6.7*um,camera_pixel_factor_y=6.7*um):
    width=np.empty((2,data.shape[2]))
    for i in range(data.shape[2]):
        p,pcov,success=fitgaussian_2D(data[:,:,i])
        fit_gaussian_2D_to_image(data[:,:,i],filename+str(i))        
    ...
    #Fit the function sigma_t^2=A+(k_b*T/m)*t^2:
    TOF_step=0.25*1e-3
    p_x=np.polyfit(np.arange(data.shape[2])*TOF_step,width[0,:],1)
    p_y=np.polyfit(np.arange(data.shape[2])*TOF_step,width[1,:],1)
    print(p_x,p_y)
...
##MOT Ladephase:
#
#filepath_MOT='/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messungen_LK/'   
#print(find_bmp_filenames(filepath_MOT))
#MOT_files=[filename for filename in find_bmp_filenames(filepath_MOT) if 'Ladephase0' in filename]
#BG_file=[filename for filename in find_bmp_filenames(filepath_MOT) if 'BG' in filename][0]

#Sort the MOT_files correctly
#Splitter='Ladephase0'
#MOT_files_split=[f.split(Splitter) for f in MOT_files]
#MOT_files_sorted=[Splitter.join(f) for f in sorted(MOT_files_split,key=lambda x: x[1])]
...
filepath_MOT_Temp='/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messdaten_Dominik/FP-Daten/1/TempMot'
MOT_Temp_files=[filename for filename in find_bmp_filenames(filepath_MOT_Temp)]
#print(MOT_Temp_files)
#BG_file_MOT_Temp=[filename for filename in find_bmp_filenames(filepath_MOT_Temp) if 'BG' in filename][0]
#Sort the MOT_files correctly

Splitter_MOT='TempMot'
MOT_Temp_files_sorted=file_sorter(MOT_Temp_files,Splitter_MOT)

#Use only the first 4 pictures as programm did error:
MOT_Temp_files_sorted=MOT_Temp_files_sorted[:6]
#print(MOT_Temp_files_sorted)
MOT_Temp_data=data_processing(MOT_Temp_files_sorted)
#print(MOT_Temp_data.shape)
plot_Temp(MOT_Temp_data,filepath_MOT_Temp+'/MOT_Temp')
#plot_Temp2(MOT_Temp_data,filepath_MOT_Temp+'/MOT_Temp')



##Melasse_Temp:
#
#filepath_Melasse_Temp='/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messdaten_Dominik/FP-Daten/1/Melasse'
#Melasse_Temp_files=[filename for filename in find_bmp_filenames(filepath_Melasse_Temp)]
##Sort the MOT_files correctly
#Splitter_Melasse='TempMot'
#Melasse_Temp_files_sorted=file_sorter(Melasse_Temp_files,Splitter_Melasse)
##Use only the first 4 pictures as programm did error:
#Melasse_Temp_files_sorted=Melasse_Temp_files_sorted[:]
#print(Melasse_Temp_files_sorted)
#Melasse_Temp_data=data_processing(Melasse_Temp_files_sorted)
