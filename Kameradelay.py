...
def data_processing(data_files, BG_data):
...
def plot_cam_delay(data):
    
    #Sum intensities of all files seperately and plot them vs. image number/time step 
    summed_data=np.sum(data,axis=(0,1))
    plt.plot((np.arange(summed_data.shape[0])-int((summed_data.shape[0]+1)/2))*0.1,summed_data)
    plt.xlabel('Kameratrigger shifted vs light pulse [ms]')
    plt.ylabel('Summed light intensity MOT [a.u.]')
    plt.tight_layout()
    plt.savefig('/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messungen_LK/2018_04_05/Camera_trigger_shifted')    

path_cam_before_light='/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messungen_LK/2018_04_05/Delay Licht nach Kamera'
path_cam_after_light='/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messungen_LK/2018_04_05/DelayLicht vor Kamera'

files_cam_before_light=[filename for filename in find_bmp_filenames(path_cam_before_light)]
files_cam_after_light=[filename for filename in find_bmp_filenames(path_cam_after_light)]
...
splitter='Delay0'
files_cam_before_light_sorted=file_sorter(files_cam_before_light, splitter)[::-1]
files_cam_after_light_sorted=file_sorter(files_cam_after_light, splitter)
total_files=files_cam_before_light_sorted+files_cam_after_light_sorted[1:]
bg_file=np.zeros((480,640))
data=data_processing(total_files, bg_file)


plot_cam_delay(data)
#print(files_cam_before_light)
##
##
##MOT Ladephase:
#
