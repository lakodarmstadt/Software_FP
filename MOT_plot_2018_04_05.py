...
##MOT Ladephase:
#
filepath_MOT='/home/lars/Dokumente/Lars_Kohfahl/Studium/PhD/Lehre/FP/Messungen_LK/2018_04_05/Ladephase'   
...
MOT_Temp_files=[filename for filename in find_bmp_filenames(filepath_MOT_Temp) if 'MOT_Temp0' in filename]
print(MOT_Temp_files)
BG_file_MOT_Temp=[filename for filename in find_bmp_filenames(filepath_MOT_Temp) if 'BG' in filename][0]
#Sort the MOT_files correctly
Splitter_Temp='MOT_Temp0'
MOT_Temp_files_split=[f.split(Splitter_Temp) for f in MOT_Temp_files]
MOT_Temp_files_sorted=[Splitter_Temp.join(f) for f in sorted(MOT_Temp_files_split,key=lambda x: x[1])]
...
MOT_Temp_data=data_processing(MOT_Temp_files_sorted,BG_file_MOT_Temp)
